...
Without `using Direction::*;`, the pattern arms would require `Direction::North`, etc.

**Rules:**

1. `using Enum::*;` imports every variant of `Enum` as an unqualified name.
2. If two glob imports expose the same name (e.g., `using A::*; using B::*;` where both have a variant `Active`), it is an ambiguity error at the import site — not at the use site.
...
```writ
fn f(x: bool?) -> bool? {
    let a: bool? = None;       // OK — None is always in scope
    let b = Some(true);        // OK — Some(expr) inferred as bool?
    match x {
        None    => None,
        Some(v) => Some(v),
    }
...
- `None` without a type annotation is an error: `let x = None;` → **E0120** "cannot infer type for `None`".

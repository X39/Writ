...
## 1.30.2 Full Dialogue Lowering Example
...
See [Section 1.15.7](16_15_entities.md#1157-entity-lowering) for the full lowering specification.

## 1.30.4 Localized Dialogue Lowering
...
## 1.30.5 Runtime Functions

The runtime must provide these core functions as root-namespace inbuilts (§1.27.4). Dialogue functions are
**transition points** — they suspend execution until the host responds (see §1.14.9, IL spec §1.14.2).
...
| `choice`             | `fn choice(options: ChoiceOption[]) -> int`                        | Present choices, **suspend** until player selects, return selected index      |
| `ChoiceOption`       | `fn ChoiceOption(label: string, key: string, body: fn() -> void) -> ChoiceOption` | Construct a choice descriptor with label, loc key, and callback  |

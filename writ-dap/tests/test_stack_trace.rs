...
fn test_stack_trace_source_span_resolves_exact_pc() {
    ...
    let module = make_module_with_spans(&[(0, 10, 1), (5, 15, 1), (10, 20, 1)]);
...
fn test_stack_trace_source_span_resolves_between_pcs() {
    ...
    assert_eq!(
        result,
        Some((5, 1)),
        "pc=7 should resolve to line 5 (largest span.pc <= 7)"
    );
    ...
    assert_eq!(
        result,
        Some((10, 1)),
        "pc=15 should resolve to line 10 (largest span.pc <= 15)"
    );
    ...
    assert_eq!(
        result,
        Some((15, 1)),
        "pc=25 should resolve to line 15 (last span)"
    );
}
...
fn test_stack_trace_source_span_returns_none_for_pc_before_all_spans() {
    ...
    assert_eq!(
        result, None,
        "pc=5 should return None (no span has pc <= 5)"
    );
}
...
fn test_stack_trace_method_name_resolved_from_string_heap() {
    ...
    assert_eq!(
        name, "my_function",
        "method name should be correctly recovered from string heap"
    );
}

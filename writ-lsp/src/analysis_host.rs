...
impl AnalysisHost {
    ...
    pub fn analyze_standalone(source: String, display_path: String) -> AnalysisResult {
        ...
        let path_refs: Vec<(FileId, &str)> = vec![(file_id, display_path.as_str())];
    ...
    pub fn analyze_project(
        ...
        let trigger_in_sources = trigger_canonical.as_ref().is_some_and(|tc| {
            source_files
                .iter()
                .any(|p| p.canonicalize().ok().as_ref() == Some(tc))
        });
        ...
        for (idx, path) in source_files.iter().enumerate() {
            ...
            let content = match (trigger_source.as_ref(), trigger_canonical.as_ref()) {
                (Some(ts), Some(tc)) if path.canonicalize().ok().as_ref() == Some(tc) => ts.clone(),
        ...
        if !trigger_in_sources && let Some(path) = trigger_file {
            let file_id = FileId(file_sources.len() as u32);
            let display_path = path.to_string();

            let content =
                trigger_source.unwrap_or_else(|| std::fs::read_to_string(path).unwrap_or_default());
        ...
        if !has_errors {
            if let (Some(typed), Some(interner)) = (&typed_ast_out, &ty_interner_out) {
                let sources_slice: Vec<(FileId, &str)> = file_sources
                    ...
                    .collect();
                let primary_source = file_sources
                    .first()
                    .map(|(_, _, s)| s.as_str())
                    .unwrap_or("");
                let runtime_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
...
fn io_error_diag(path: &str, err: &std::io::Error) -> Diagnostic {
    ...
    Diagnostic::error("E9998", format!("failed to read {}: {}", path, err))
...
fn config_error_diag(err: &writ_compiler::config::ConfigError) -> Diagnostic {
    ...
    Diagnostic::error("E9997", format!("project configuration error: {}", err))
...
fn try_runtime_diagnostic(
    ...
    let bytes =
        writ_compiler::emit_bodies(typed_ast, interner, asts, true, sources, &active_conditions)
            .ok()?;
...
mod tests {
    ...
    fn test_analyze_standalone_valid() {
        let result =
            AnalysisHost::analyze_standalone("fn main() {}".to_string(), "test.writ".to_string());
    ...
    fn test_analyze_standalone_type_error() {
        ...
        assert!(
            has_error,
            "Expected at least one error diagnostic for type mismatch"
        );
    }
...
"#,
        ...
        let file_ids: HashSet<u32> = result
            .file_sources
            .iter()
            .map(|(fid, _, _)| fid.0)
            .collect();
        assert_eq!(
            file_ids.len(),
            2,
            "Expected distinct FileIds for each discovered file"
        );
...
"#,
    ...
    fn test_analyze_project_missing_toml_with_trigger() {
        ...
        assert!(
            errors.is_empty(),
            "Expected no errors for valid trigger file"
        );
    ...
    fn test_hover_standalone_finds_variable() {
        ...
        assert!(
            expr.is_some(),
            "expr_at_offset should find the `x` usage at offset {}",
            x_usage_offset
        );
    ...
    fn test_hover_standalone_finds_binding() {
        ...
        let binding =
            crate::queries::binding_at_offset(typed_ast, let_x_offset, type_env, FileId(0));
        assert!(
            binding.is_some(),
            "binding_at_offset should find `x` at the let declaration"
        );
        ...
        assert!(
            ty_str.contains("int"),
            "binding type should be int, got: {}",
            ty_str
        );
    }
    ...
    fn test_hover_project_mode_correct_file_id() {
        ...
        fs::write(
            src_dir.join("a.writ"),
            "pub fn greet_all_the_people_in_the_world() -> int { 99999 }",
        )
        .unwrap();
        // b.writ: short
        ...
        assert!(
            expr_in_a.is_some(),
            "should find expression in a.writ with a's FileId"
        );
        ...
        assert!(
            cross.is_none(),
            "should NOT find a.writ expression with b's FileId"
        );
    ...
    fn test_resolve_trigger_file_id_standalone() {
        ...
        let path = std::env::temp_dir()
            .join("writ_lsp_test_resolve_trigger_file_id")
            .join("foo.writ");
        ...
        let fid = crate::backend::resolve_trigger_file_id(&file_sources, &uri.to_string(), &uri);
        assert_eq!(
            fid,
            FileId(0),
            "should resolve to FileId(0) for matching path"
        );
    }
    ...
    fn test_goto_def_type_annotation() {
        ...
        let def_id =
            crate::queries::type_ann_def_id_at_offset(typed_ast, type_foo_offset, FileId(0));
        assert!(
            def_id.is_some(),
    ...
    fn test_find_refs_from_declaration() {
        ...
        assert!(
            def_id.is_some(),
            "def_at_offset should find helper at its declaration"
        );

        // collect_references should find the call sites
        let refs =
            crate::queries::collect_references(typed_ast, def_id.unwrap(), &typed_ast.def_map);
        assert!(
            refs.len() >= 2,
    ...
    fn test_file_id_filtering_skips_other_files() {
        ...
        fs::write(
            src_dir.join("a.writ"),
            "pub fn a_very_long_function_name_for_testing() -> int { 123456789 }",
        )
        .unwrap();
        fs::write(src_dir.join("b.writ"), "fn main() {}").unwrap();
    ...
    fn test_hover_fn_call_shows_signature() {
        ...
        assert!(
            expr.is_some(),
            "expr_at_offset should find expression at add call, offset {}",
            call_offset
        );
        ...
        assert!(
            hover.contains("fn"),
            "hover should contain 'fn', got: {}",
            hover
        );
        assert!(
            hover.contains("int"),
            "hover should contain 'int' (param/return type), got: {}",
            hover
        );
    }
    ...
    fn test_hover_literal_shows_type() {
        ...
        assert!(
            expr.is_some(),
            "should find literal expression at offset {}",
            lit_offset
        );
        ...
        assert!(
            hover.contains("int"),
            "hover for integer literal should contain 'int', got: {}",
            hover
        );
    }
    ...
    fn test_hover_string_literal_shows_type() {
        ...
        assert!(
            ...
            "hover for string literal should contain 'string', got: {}",
            hover
        );
    }
    ...
    fn test_hover_bool_literal_shows_type() {
        ...
        assert!(
            hover.contains("bool"),
            "hover for bool literal should contain 'bool', got: {}",
            hover
        );
    }
    ...
    fn test_hover_field_access_shows_field_type() {
        ...
        assert!(
            expr.is_some(),
            "should find expression at the field access '.x'"
        );
        ...
        assert!(
            hover.contains("int"),
            "hover for field access f.x should contain 'int', got: {}",
            hover
        );
    }
    ...
    fn test_goto_def_fn_call() {
        ...
        let call_offset = src
            .rfind("target()")
            .expect("should find target() call in main");
        let expr = crate::queries::expr_at_offset(typed_ast, call_offset, FileId(0));
        assert!(
            expr.is_some(),
            "should find expression at target() call site"
        );
        ...
        assert_eq!(
            entry.name, "target",
            "DefEntry name should be 'target', got '{}'",
            entry.name
        );
    }
    ...
    fn test_goto_def_struct_new() {
        ...
        assert!(
            expr.is_some(),
            "should find expression at 'Bar' in 'new Bar' site"
        );
        ...
        assert_eq!(
            entry.name, "Bar",
            "DefEntry name should be 'Bar', got '{}'",
            entry.name
        );
    }
    ...
    fn test_goto_def_cross_file() {
        ...
        let result = AnalysisHost::analyze_project(&tmp, None, None);
        let typed_ast = result
            .typed_ast
            .as_ref()
            .expect("should produce typed_ast for cross-file project");

        // Look up 'helper' in the def_map -- it should exist as a public definition
        let helper_def_id = typed_ast
            .def_map
            .by_fqn
            .values()
            .copied()
    ...
    fn test_find_refs_struct_usage() {
        let src =
            "pub struct Foo {} fn main() { let a: Foo = new Foo {}; let b: Foo = new Foo {}; }";
        let result = AnalysisHost::analyze_standalone(src.to_string(), "test.writ".to_string());
        let typed_ast = result.typed_ast.as_ref().expect("should produce typed_ast");
        ...
        assert!(
            def_id.is_some(),
            "should find Foo definition at its declaration site"
        );
    ...
    fn test_find_refs_no_false_positives() {
        ...
        let alpha_def_id =
            crate::queries::def_at_offset(&typed_ast.def_map, alpha_offset, FileId(0));
        ...
        let alpha_refs = crate::queries::collect_references(
            typed_ast,
            alpha_def_id.unwrap(),
            &typed_ast.def_map,
        );
        let beta_refs =
            crate::queries::collect_references(typed_ast, beta_def_id.unwrap(), &typed_ast.def_map);
    ...
    fn test_identifier_completions_include_user_fn() {
        ...
        assert!(
            fn_item.is_some(),
            "completions should include 'my_custom_fn'"
        );
        assert_eq!(
            fn_item.unwrap().kind,
    ...
    fn test_dot_completion_struct_method() {
        ...
        assert!(
            labels.contains(&"x"),
            "dot completions should include field 'x', got: {:?}",
            labels
        );
        assert!(
            labels.contains(&"y"),
            "dot completions should include field 'y', got: {:?}",
            labels
        );
        assert!(
            labels.contains(&"len"),
            "dot completions should include method 'len', got: {:?}",
            labels
        );
    }
    ...
    fn test_dot_completion_array_methods() {
        ...
        let items =
            crate::queries::build_dot_completions(array_ty, interner, &typed_ast.def_map, type_env);

        let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
        assert!(
            labels.contains(&"push"),
            "array dot completions should include 'push', got: {:?}",
            labels
        );
        assert!(
            labels.contains(&"pop"),
            "array dot completions should include 'pop', got: {:?}",
            labels
        );
        assert!(
            labels.contains(&"len"),
            "array dot completions should include 'len', got: {:?}",
            labels
        );
        assert!(
            labels.contains(&"is_empty"),
            "array dot completions should include 'is_empty', got: {:?}",
            labels
        );
    }
    ...
    fn test_signature_help_multi_param() {
        ...
        let help = crate::queries::build_signature_help(
            src_static, comma_pos, typed_ast, interner, type_env,
        );
        assert!(
            help.is_some(),
            "should return signature help for multi-param fn"
        );

        let help = help.unwrap();
        let sig = &help.signatures[0];
        assert_eq!(
            sig.parameters.as_ref().unwrap().len(),
            3,
    ...
    fn test_signature_help_no_args_fn() {
        ...
        let src_static: &'static str = Box::leak(src.to_string().into_boxed_str());
        let help = crate::queries::build_signature_help(
            src_static, paren_pos, typed_ast, interner, type_env,
        );
        assert!(
            help.is_some(),
            "should return signature help for no-args fn"
        );
    ...
    fn test_semantic_tokens_fn_declaration() {
        ...
        assert!(
            func_token.is_some(),
            "should have a semantic token at 'my_func' position"
        );
        // TOKEN_TYPE_FUNCTION = 5
        assert_eq!(
            func_token.unwrap().token_type,
            5,
    ...
    fn test_semantic_tokens_class_declaration() {
        ...
        assert!(
            class_token.is_some(),
            "should have a semantic token at 'MyClass' position"
        );
        // TOKEN_TYPE_TYPE = 1
        assert_eq!(
            class_token.unwrap().token_type,
            1,
    ...
    fn test_semantic_tokens_enum_declaration() {
        ...
        assert!(
            dir_token.is_some(),
            "should have a semantic token at 'Dir' position"
        );
        // TOKEN_TYPE_TYPE = 1
        assert_eq!(
            dir_token.unwrap().token_type,
            1,
    ...
    fn test_position_to_byte_offset_end_of_line() {
        ...
        assert!(
            offset.is_some(),
            "should return a valid byte offset for end of line content"
        );
        ...
        assert_eq!(
            offset.unwrap(),
            expected,
            "offset should point to ';' at byte {}",
            expected
        );
    }
    ...
    fn test_position_to_byte_offset_empty_line() {
        ...
        assert!(
            offset.is_some(),
            "should return a valid byte offset for empty line"
        );
    ...
    fn test_span_to_range_multiline() {
        ...
        let span = chumsky::span::SimpleSpan {
            start: let_start,
            end: semi_end,
            context: (),
        };
        ...
        assert_eq!(
            ...
            "span start character should be 4 (after 4 spaces)"
        );
    }
    ...
    fn test_runtime_crash_force_unwrap_shows_stacktrace() {
        let handle = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                let result =
                    AnalysisHost::analyze_standalone(src.to_string(), "test.writ".to_string());
                let runtime_diags: Vec<_> = result
                    .diagnostics
                    .iter()
                ...
                assert_eq!(
                    runtime_diags.len(),
                    1,
                    "expected one runtime crash diagnostic"
                );
                let msg = &runtime_diags[0].message;
                assert!(
                    msg.contains("Runtime crash"),
                    "should contain 'Runtime crash': {}",
                    msg
                );
                assert!(
                    msg.contains("main"),
                    "stack trace should mention 'main': {}",
                    msg
                );
            })
            .unwrap();
    ...
    fn test_no_runtime_diagnostic_when_compile_errors() {
        ...
        let runtime_diags: Vec<_> = result
            ...
            .collect();
        assert!(
            runtime_diags.is_empty(),
            "should not attempt runtime when compile errors exist"
        );
    }
    ...
    fn test_no_runtime_diagnostic_clean_script() {
        let handle = std::thread::Builder::new()
            ...
            .spawn(|| {
                ...
                assert!(
                    runtime_diags.is_empty(),
                    "clean script should produce no runtime diagnostics"
                );
            })
            .unwrap();
        handle.join().unwrap();
...
"#
        ...
        assert!(
            ...
            "E0123 should mention 'MyClass', got: {}",
            msg
        );
        assert!(
            !msg.contains("impl#"),
            "E0123 should not mention 'impl#', got: {}",
            msg
        );
    }

    ...
    I: ValueInput<'src, Token = Token<'src>, Span = Span>,
{
    ...
    >| {
        ...
        let qualified_name_glob = qualified_name.clone()
            .then(
        ...
        let class_on_hook = just(Token::KwOn)
            ...
            .map_with(|(event, body), e| {
                (cst::ClassMember::OnHook { event, body }, e.span())
            });

        let class_field_member = struct_field.clone()
            .map(|(sf, span)| {
                (cst::ClassMember::Field(sf), span)
            });

        let class_member = choice((
            class_on_hook,
            class_field_member,
        ));
        ...
        let attrs_vis_decl = attrs.clone()
            ...
            .then(choice((
                ...
                // class -> Item::Class
                class_decl.map_with(|cd, e| {
                    (cst::Item::Class((cd, e.span())), e.span())
                }),
                // enum -> Item::Enum

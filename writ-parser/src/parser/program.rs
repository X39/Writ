//! NOTE: program.rs exceeds the 500-line target (~2,960 lines). This is a documented exception:
//! Chumsky's recursive() combinator creates a single closure scope where all grammar productions
//! must be visible simultaneously. Splitting the recursive closure is not structurally sound.
...
use super::generic_params::generic_params;
use super::type_expr::type_expr;
use super::{ExprPostfix, Span};
    ...
    while pos < bytes.len() {
        if pos + 1 < bytes.len() && bytes[pos] == b'{' && bytes[pos + 1] == b'{' {
            ...
            if pos > text_start {
                ...
                let span = SimpleSpan::from((content_start + text_start)..(content_start + pos));
            ...
            if let Some(parsed_expr) = parse_expr_from_source(expr_text) {
                ...
                let seg_span =
                    SimpleSpan::from((content_start + expr_start - 1)..(content_start + pos + 1));
                segments.push((cst::StringSegment::Expr(Box::new(adjusted)), seg_span));
            }
    ...
    while pos < bytes.len() {
        if pos + 1 < bytes.len() && bytes[pos] == b'{' && bytes[pos + 1] == b'{' {
            ...
            if pos > seg_start {
                ...
                let span = SimpleSpan::from((text_start + seg_start)..(text_start + pos));
            ...
            if let Some(parsed_expr) = parse_expr_from_source(expr_text) {
                ...
                let seg_span =
                    SimpleSpan::from((text_start + expr_start - 1)..(text_start + pos + 1));
                segments.push((cst::DlgTextSegment::Expr(Box::new(adjusted)), seg_span));
            }
                ...
                || (bytes[pos + 1] == b'\r' && pos + 2 < bytes.len() && bytes[pos + 2] == b'\n'))
        {
            ...
            let space_span =
                SimpleSpan::from((text_start + continuation_start)..(text_start + pos));
...
fn parse_expr_from_source<'src>(expr_src: &'src str) -> Option<cst::Spanned<cst::Expr<'src>>> {
    ...
    let token_stream = Stream::from_iter(non_trivia).map(eoi, |(t, s): (_, _)| (t, s));
    ...
    output.and_then(|items| {
        items.into_iter().next().map(|(item, span)| match item {
            cst::Item::Stmt((cst::Stmt::Expr(expr), _)) => expr,
            _ => (cst::Expr::NullLit, span), // fallback, shouldn't happen
        })
    })
}
...
#[allow(clippy::type_complexity)] // parser combinator closures have inherently complex types; splitting would reduce readability
pub fn program_parser<'src, I>(
    src: &'src str,
) -> impl Parser<'src, I, Vec<cst::Spanned<cst::Item<'src>>>, extra::Err<Rich<'src, Token<'src>, Span>>>
+ Clone
+ 'src
where
...
{
    recursive(
        |items: Recursive<dyn Parser<'src, I, Vec<cst::Spanned<cst::Item<'src>>>, _> + '_>| {
            // =============================================================
            ...
            let expr = {
                ...
                recursive(
                    ...
                    >| {
                        ...
                        let raw_string_lit =
                            select! { Token::RawStringLit => () }.map_with(move |_, e| {
                        ...
                        let ident_or_path = just(Token::ColonColon)
                            .or_not()
                            .then(
                                ident_token
                        ...
                        let paren_expr = expr
                            ...
                            .map_with(|inner: cst::Spanned<cst::Expr<'src>>, e| {
                                (inner.0, e.span())
                            });
                        ...
                        let if_expr = recursive(
                            ...
                            >| {
                                ...
                                let if_let = just(Token::KwIf)
                                    ...
                                    .ignore_then(super::pattern::pattern())
                                    .then_ignore(just(Token::Eq))
                                    .then(expr.clone())
                                    .then(block.clone())
                                    .then(
                                        just(Token::KwElse)
                                            .ignore_then(if_expr.clone().or(
                                                block.clone().map_with(|b, e| {
                        ...
                        let match_arm = super::pattern::pattern()
                            .then_ignore(just(Token::FatArrow))
                            .then(block.clone())
                            .map_with(|(pat, body), e| {
                                (cst::MatchArm { pattern: pat, body }, e.span())
                            });
                        ...
                        let lambda_param = select! { Token::Ident(name) => name }
                            ...
                            .map_with(|(name, ty), e| (cst::LambdaParam { name, ty }, e.span()));
                        ...
                        let defer_expr = just(Token::KwDefer)
                            ...
                            .map_with(|block_expr, extra| {
                                (cst::Expr::Defer(Box::new(block_expr)), extra.span())
                            });
                        ...
                        let new_field = select! { Token::Ident(name) => name }
                            ...
                            .then(expr.clone())
                            .map_with(|(name, value), e| (cst::NewField { name, value }, e.span()));
                        ...
                        let gc_arg = select! { Token::Ident(name) => name }
                            ...
                            .or_not()
                            .then(expr.clone())
                            .map_with(|(name, value), e| (cst::Arg { name, value }, e.span()));
                        ...
                        let postfix_chain = atom.foldl_with(
                            choice((
                                ...
                                // `..` is not consumed by the full expression parser.
                                {
                                    ...
                                    let start_only_range = range_operand
                                        .clone()
                                        .then(range_kind.clone())
                                        .map_with(|(start, kind), e| {
                                            (
                                                cst::Expr::Range(Some(Box::new(start)), kind, None),
                                                e.span(),
                                            )
                                        });
                                    // Half-open range (end only): ..end, ..=end
                                    let end_only_range = range_kind
                                        .clone()
                                        .then(range_operand)
                                        .map_with(|(kind, end), e| {
                                            (
                                                cst::Expr::Range(None, kind, Some(Box::new(end))),
                                                e.span(),
                                            )
                                        });
                                    // Full unbounded range: .., ..=
                                    let unbounded_range = range_kind.map_with(|kind, e| {
                            ...
                            .repeated(),
                            |left: cst::Spanned<cst::Expr<'src>>, postfix: ExprPostfix<'src>, e| {
                                let span = e.span();
                                match postfix {
                                    ...
                                    ExprPostfix::Bracket(inner) => (
                                        cst::Expr::BracketAccess(Box::new(left), Box::new(inner)),
                                        span,
                                    ),
                                    ExprPostfix::GenericMethod(field, type_args, call_args) => {
                                        let member_span =
                        ...
                        let pratt_expr = postfix_chain.pratt((
                            ...
                            prefix(
                                ...
                                |_, rhs: cst::Spanned<cst::Expr<'src>>, e| {
                                    (
                                        cst::Expr::UnaryPrefix(cst::PrefixOp::Neg, Box::new(rhs)),
                                        e.span(),
                                    )
                                },
                            ),
                            prefix(14, just(Token::Bang), |_, rhs, e| {
                                (
                                    cst::Expr::UnaryPrefix(cst::PrefixOp::Not, Box::new(rhs)),
                                    e.span(),
                                )
                            }),
                            ...
                            infix(left(12), just(Token::Star), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Mul, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            infix(left(12), just(Token::Slash), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Div, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            infix(left(12), just(Token::Percent), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Mod, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            // --- Additive: +, - ---
                            infix(left(11), just(Token::Plus), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Add, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            infix(left(11), just(Token::Minus), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Sub, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            // --- Shift: <<, >> ---
                            ...
                            infix(
                                left(10),
                                just(Token::Lt).then(just(Token::Lt)),
                                |l, _, r, e| {
                                    (
                                        cst::Expr::Binary(
                                            Box::new(l),
                            ...
                            infix(
                                left(10),
                                just(Token::Gt).then(just(Token::Gt)),
                                |l, _, r, e| {
                                    (
                                        cst::Expr::Binary(
                                            Box::new(l),
                            ...
                            infix(left(8), just(Token::Lt), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Lt, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            infix(left(8), just(Token::Gt), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Gt, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            ...
                            infix(left(7), just(Token::EqEq), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Eq, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            ...
                            infix(left(4), just(Token::AmpAmp), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::And, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            // --- Logical OR: || ---
                            infix(left(3), just(Token::PipePipe), |l, _, r, e| {
                                (
                                    cst::Expr::Binary(Box::new(l), cst::BinaryOp::Or, Box::new(r)),
                                    e.span(),
                                )
                            }),
                            // --- Assignment: =, +=, -=, *=, /=, %= ---
                        ...
                        pratt_expr
                            ...
                            .map_with(
                                |pair: (
                                    ...
                                    Option<(cst::RangeKind, Option<cst::Spanned<cst::Expr<'src>>>)>,
                                ),
                                 e| {
                                    let (lhs, range) = pair;
            ...
            let dlg_code_stmt = {
                ...
                let expr_s = expr
                    .clone()
                    .then_ignore(just(Token::Semi))
                    .map_with(|expression, e| (cst::Stmt::Expr(expression), e.span()));
            ...
            let dlg_body = recursive(
                |dlg_lines: Recursive<
                    dyn Parser<'src, I, Vec<cst::Spanned<cst::DlgLine<'src>>>, _> + '_,
                >| {
                    // --- Localization key parser: #ident ---
                    let loc_key = just(Token::Hash).ignore_then(
                        select! { Token::Ident(name) => name }.map_with(|n, e| (n, e.span())),
                    );
                    ...
                    let dlg_if = recursive(
                        |dlg_if_rec: Recursive<
                            dyn Parser<'src, I, cst::Spanned<cst::DlgIf<'src>>, _> + '_,
                        >| {
                            expr.clone()
                                .then(
                                    dlg_lines
                                    ...
                                    just(Token::KwElse)
                                        .ignore_then(choice((
                                            ...
                                            just(Token::KwIf)
                                                ...
                                                .map_with(|inner, e| {
                                                    (cst::DlgElse::ElseIf(inner.0), e.span())
                                                }),
                                            // else { ... }
                                            dlg_lines
                                                .clone()
                    ...
                    let dlg_match_arm = super::pattern::pattern()
                        .then_ignore(just(Token::FatArrow))
                        .then(
                            dlg_lines
                                .clone()
                        ...
                        .map_with(|(pat, body), e| {
                            (cst::DlgMatchArm { pattern: pat, body }, e.span())
                        });
                    ...
                    let dlg_choice = choice_arm
                        ...
                        .collect::<Vec<_>>()
                        .delimited_by(just(Token::LBrace), just(Token::RBrace))
                        .map_with(|arms, e| (cst::DlgChoice { arms }, e.span()));

                    // --- Dollar escape parser ---
                    // $ if | $ match | $ choice | $ { block } | $ stmt;
                    let dlg_escape = just(Token::Dollar).ignore_then(choice((
                        // $ if condition { dlg } [else { dlg }]
                        just(Token::KwIf)
                            .ignore_then(dlg_if)
                            .map_with(|dif, e| (cst::DlgLine::If(dif), e.span())),
                        // $ match expr { arms }
                        just(Token::KwMatch)
                            .ignore_then(dlg_match)
                            .map_with(|dm, e| (cst::DlgLine::Match(dm), e.span())),
                        // $ choice { arms }
                        // "choice" is Ident("choice"), not a keyword
                        select! { Token::Ident("choice") => () }
                            .ignore_then(dlg_choice)
                            .map_with(|dc, e| (cst::DlgLine::Choice(dc), e.span())),
                    ...
                    let brace_group = recursive(
                        |brace_group: Recursive<
                            dyn Parser<'src, I, Vec<(Token<'src>, Span)>, _> + '_,
                        >| {
                            just(Token::LBrace)
                                ...
                                .then(
                                    choice((
                                        ...
                                        any()
                                            .and_is(
                                                just(Token::LBrace).or(just(Token::RBrace)).not(),
                                            )
                                ...
                                .then(
                                    just(Token::RBrace).map_with(|t: Token<'src>, e| (t, e.span())),
                                )
                    ...
                    let extract_text_and_loc_key = move |tokens: &[(Token<'src>, Span)]| -> (
                        ...
                        let (text_tokens_slice, loc_key) = if tokens.len() >= 2 {
                            ...
                            if matches!(second_last.0, Token::Hash) {
                                if let Token::Ident(name) = last.0 {
                                    (&tokens[..tokens.len() - 2], Some((name, last.1)))
                                } else {
                                    (tokens, None)
                                }
                            } else {
                        ...
                        let text_end = text_tokens_slice[text_tokens_slice.len() - 1].1.end;

                        let segments = split_dlg_text_segments(src, text_start, text_end);
                    ...
                    let speaker_line = just(Token::At)
                        ...
                        .map_with(move |(speaker, maybe_tokens), e| match maybe_tokens {
                            None => {
                                // No text tokens -- standalone speaker tag
                                (cst::DlgLine::SpeakerTag(speaker), e.span())
                            }
                            Some(tokens) => {
                                let (text, loc_key) = extract_text_and_loc_key(&tokens);
                                (
                    ...
                    let text_line =
                        text_tokens
                            .clone()
                            .map_with(move |tokens: Vec<(Token<'src>, Span)>, e| {
                                let (text, loc_key) = extract_text_and_loc_key(&tokens);
                                (cst::DlgLine::TextLine { text, loc_key }, e.span())
                            });

                    // --- Dialogue line: one of the dialogue forms ---
                    // Order: speaker (@), escape ($), transition (->), text
                    let dlg_line = choice((speaker_line, dlg_escape, transition, text_line));
            ...
            let dlg_decl = just(Token::KwDlg)
                .ignore_then(select! { Token::Ident(name) => name }.map_with(|n, e| (n, e.span())))
                .then(dlg_param_list.or_not())
                .then(dlg_body.delimited_by(just(Token::LBrace), just(Token::RBrace)))
            ...
            let while_stmt = just(Token::KwWhile)
                .ignore_then(expr.clone())
                .then(block.clone())
                .map_with(|(condition, body), e| (cst::Stmt::While { condition, body }, e.span()));
            ...
            let expr_stmt = expr
                ...
                .map_with(|(expression, _semi), e| (cst::Stmt::Expr(expression), e.span()));
            ...
            let transition_stmt = just(Token::Arrow)
                ...
                .map_with(|(target, args), e| {
                    (
                        cst::Stmt::Transition((cst::DlgTransition { target, args }, e.span())),
                        e.span(),
                    )
                });
            ...
            let attr_arg = choice((
                ...
                select! { Token::Ident(name) => name }
                    ...
                    .then_ignore(just(Token::Eq))
                    .then(expr.clone())
                    .map_with(|(name, value), e| (cst::AttrArg::Named(name, value), e.span())),
                // Positional arg: expr
                expr.clone()
                    .map_with(|value, e| (cst::AttrArg::Positional(value), e.span())),
            ));
            ...
            let fn_decl = just(Token::KwFn)
                ...
                .then(block.clone())
                .map(
                    |((((name, generics), params), return_type), body)| cst::FnDecl {
                        attrs: Vec::new(),
                        vis: None,
                        name,
                        generics,
            ...
            let namespace_decl = just(Token::KwNamespace)
                ...
                .map_with(|(name, maybe_body), e| {
                    ...
                    (cst::Item::Namespace((decl, e.span())), e.span())
                });

            // --- using_decl parser ---
            // using [alias =] qualified::name[::*] ;
            // The optional ::* at the end signals a glob import (e.g., using Status::*;).
            // When present, "*" is appended to the path segments so the resolver can detect it.
            let qualified_name_glob = qualified_name
                .clone()
                .then(
                    just(Token::ColonColon)
                        .ignore_then(just(Token::Star).map_with(|_, e| ("*", e.span())))
                        .or_not(),
                )
                .map(|(mut path, glob)| {
                    if let Some(star) = glob {
                        path.push(star);
                    }
                    path
                });
            ...
            let using_decl = just(Token::KwUsing)
                ...
                .then(qualified_name_glob)
                .then_ignore(just(Token::Semi))
                .map_with(|(alias, path), e| {
                    (
                        cst::Item::Using((cst::UsingDecl { alias, path }, e.span())),
                        e.span(),
                    )
                });
            ...
            // Rejects `on create` and `on finalize` with a diagnostic (parse-and-report pattern).
            ...
            .then(block.clone())
            .validate(|((event_name, event_span), body), e, emitter| {
                if event_name == "create" || event_name == "finalize" {
                    emitter.emit(Rich::custom(
                        event_span,
                        "lifecycle hooks 'on create' and 'on finalize' are not allowed on value-type structs; use 'class' for reference types with hooks".to_string(),
                    ));
                }
                (cst::StructMember::OnHook { event: (event_name, event_span), body }, e.span())
            });

            // Struct field wrapped as StructMember
            let struct_field_member = struct_field
                .clone()
                .map(|(sf, span)| (cst::StructMember::Field(sf), span));

            // Struct member: hook or field (hooks first to match `on` keyword before field ident)
            let struct_member = choice((struct_on_hook, struct_field_member));
            ...
            // --- class_decl parser ---
            // [vis] class Name [<generics>] { [vis] field: type [= default], ... }
            // Classes accept all lifecycle hooks (create, finalize, serialize, deserialize).
            let class_on_hook = just(Token::KwOn)
                .ignore_then(select! { Token::Ident(name) => name }.map_with(|n, e| (n, e.span())))
                .then(block.clone())
                .map_with(|(event, body), e| (cst::ClassMember::OnHook { event, body }, e.span()));

            let class_field_member = struct_field
                .clone()
                .map(|(sf, span)| (cst::ClassMember::Field(sf), span));

            let class_member = choice((class_on_hook, class_field_member));

            let class_decl = just(Token::KwClass)
                .ignore_then(select! { Token::Ident(name) => name }.map_with(|n, e| (n, e.span())))
                .then(generic_params().or_not())
                .then(
                    class_member
                ...
                .map(|((name, generics), members)| cst::ClassDecl {
                    attrs: Vec::new(),
                    vis: None,
                    name,
                    generics,
            ...
            let enum_variant = select! { Token::Ident(name) => name }
                ...
                .map_with(|(name, fields), e| (cst::EnumVariant { name, fields }, e.span()));
            ...
            let contract_fn_member = fn_sig
                .clone()
                .map_with(|sig, e| (cst::ContractMember::FnSig(sig), e.span()));
            ...
            let impl_member = choice((op_decl, impl_fn_member));
            ...
            let use_field = select! { Token::Ident(name) => name }
                ...
                .then_ignore(just(Token::Colon))
                .then(expr.clone())
                .map_with(|(name, value), e| (cst::UseField { name, value }, e.span()));
            ...
            let entity_member = choice((entity_use, entity_on, entity_fn, entity_property));
            ...
            let component_field = struct_field
                .clone()
            ...
            let component_member = choice((component_fn, component_field.clone()));
            ...
            let extern_fn = just(Token::KwFn)
                ...
                .then_ignore(just(Token::Semi))
                .map_with(
                    |((((qualifier, name), generics), params), return_type), e| {
                        cst::ExternDecl::Fn(
                            None,
                            (
                                cst::FnSig {
            ...
            let extern_decl =
                just(Token::KwExtern).ignore_then(choice((extern_fn, extern_component)));
            ...
            let attrs_vis_decl = attrs
                ...
                .then(choice((
                    ...
                    // const -> Item::Const
                    const_decl.map_with(|cd, e| (cst::Item::Const((cd, e.span())), e.span())),
                    // global -> Item::Global
                    global_decl.map_with(|gd, e| (cst::Item::Global((gd, e.span())), e.span())),
                    // struct -> Item::Struct
                    struct_decl.map_with(|sd, e| (cst::Item::Struct((sd, e.span())), e.span())),
                    // class -> Item::Class
                    class_decl.map_with(|cd, e| (cst::Item::Class((cd, e.span())), e.span())),
                    // enum -> Item::Enum
                    enum_decl.map_with(|ed, e| (cst::Item::Enum((ed, e.span())), e.span())),
                    // contract -> Item::Contract
                    contract_decl.map_with(|cd, e| (cst::Item::Contract((cd, e.span())), e.span())),
                    // entity -> Item::Entity
                    entity_decl.map_with(|ed, e| (cst::Item::Entity((ed, e.span())), e.span())),
                    ...
                    component_decl.validate(|_cd, e, emitter| {
                        ...
                        (
                            cst::Item::Stmt((
                                cst::Stmt::Expr((cst::Expr::Error, e.span())),
                                e.span(),
                            )),
                            e.span(),
                        )
                    ...
                    // dlg -> Item::Dlg
                    dlg_decl.map_with(|dd, e| (cst::Item::Dlg(dd), e.span())),
                    // attribute -> Item::Attribute
                    attribute_decl
                        .map_with(|ad, e| (cst::Item::Attribute((ad, e.span())), e.span())),
                )))
            ...
            let stmt_item = stmt.map_with(|s, e| (cst::Item::Stmt(s), e.span()));

            // impl_decl -> Item::Impl (no attrs/vis prefix; impl keyword is unique)
            let impl_item = impl_decl.map_with(|id, e| (cst::Item::Impl((id, e.span())), e.span()));

            // extern_decl -> Item::Extern (with optional attrs and visibility prefix)
            let extern_item = attrs
                .clone()
                .then(visibility.clone().or_not())
                .then(extern_decl)
            ...
            item.recover_with(skip_then_retry_until(
                nested_delimiters(
                    Token::LBrace,
                    Token::RBrace,
                    [
                    ...
                    |span| {
                        (
                            cst::Item::Stmt((cst::Stmt::Expr((cst::Expr::Error, span)), span)),
                            span,
                        )
                    },
                )
...
pub fn parse<'src>(
    ...
    let non_trivia: Vec<(Token<'src>, Span)> = tokens
        ...
        .collect();
    let token_stream = Stream::from_iter(non_trivia).map(eoi, |(t, s): (_, _)| (t, s));
    let (output, mut errors) = program_parser(src).parse(token_stream).into_output_errors();

    // Post-parse validation: reject top-level expression statements.
    // Bare expressions at the top level (e.g. `asd asd`) are not declarations.
    // We emit an error here (after parsing) rather than in the parser itself
    // because `stmt_item` is reused inside block contexts where expressions ARE valid.
    if let Some(items) = &output {
        for (item, item_span) in items {
            if let cst::Item::Stmt((cst::Stmt::Expr(_), _)) = item {

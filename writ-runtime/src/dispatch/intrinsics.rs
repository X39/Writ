...
pub(super) fn execute_intrinsic(
    ...
    match id {
        ...
        IntrinsicId::FloatAdd => {
            let a = helpers::extract_float(
                &ctx.task.call_stack.last().unwrap().registers[r_obj as usize],
            );
            let b = helpers::extract_float(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 1],
            );
            let frame = ctx.task.call_stack.last_mut().unwrap();
        ...
        // ── Bool ──────────────────────────────────────────────
        IntrinsicId::BoolEq => {
            let a = helpers::extract_bool(
                &ctx.task.call_stack.last().unwrap().registers[r_obj as usize],
            );
            let b = helpers::extract_bool(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 1],
            );
            let frame = ctx.task.call_stack.last_mut().unwrap();
            frame.registers[r_dst as usize] = Value::Bool(a == b);
        ...
        IntrinsicId::StringAdd => {
            let href_a = helpers::extract_ref(
                &ctx.task.call_stack.last().unwrap().registers[r_obj as usize],
            );
            let href_b = helpers::extract_ref(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 1],
            );
            let sa = match ctx.heap.read_string(href_a) {
                Ok(s) => s.to_string(),
                Err(_) => {
                    return ExecutionResult::Crash("StringAdd: left operand not a string".into());
                }
            };
            let sb = match ctx.heap.read_string(href_b) {
                Ok(s) => s.to_string(),
                Err(_) => {
                    return ExecutionResult::Crash("StringAdd: right operand not a string".into());
                }
            };
        ...
        IntrinsicId::StringIndexChar => {
            let href = helpers::extract_ref(
                &ctx.task.call_stack.last().unwrap().registers[r_obj as usize],
            );
            let idx = helpers::extract_int(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 1],
            ) as usize;
        ...
        IntrinsicId::StringIntoInt => {
            ...
            let v: i64 = match s.trim().parse() {
                ...
                Err(_) => {
                    return ExecutionResult::Crash(format!(
                        "string.into_int(): cannot parse {:?} as int",
                        s
                    ));
                }
            };
        ...
        IntrinsicId::StringIntoFloat => {
            ...
            let s = match ctx.heap.read_string(href) {
                Ok(s) => s.to_string(),
                Err(_) => {
                    return ExecutionResult::Crash("string.into_float(): not a string".into());
                }
            };
            let v: f64 = match s.trim().parse() {
                ...
                Err(_) => {
                    return ExecutionResult::Crash(format!(
                        "string.into_float(): cannot parse {:?} as float",
                        s
                    ));
                }
            };
        ...
        IntrinsicId::StringIntoBool => {
            ...
            let v: bool = match s.trim() {
                ...
                other => {
                    return ExecutionResult::Crash(format!(
                        "string.into_bool(): cannot parse {:?} as bool",
                        other
                    ));
                }
            };
        ...
        IntrinsicId::ArrayIndex => {
            let arr_ref = helpers::extract_ref(
                &ctx.task.call_stack.last().unwrap().registers[r_obj as usize],
            );
            let idx = helpers::extract_int(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 1],
        ...
        IntrinsicId::ArraySlice => {
            ...
            let start = helpers::extract_int(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 1],
            ) as usize;
            let end = helpers::extract_int(
                &ctx.task.call_stack.last().unwrap().registers[r_base as usize + 2],
            ) as usize;
            match ctx.heap.get_object(arr_ref) {
                Ok(HeapObject::Array {
                    elem_type,
        ...
        IntrinsicId::FloatGetType => {
            let href = ctx
                .reflection
                .get_or_alloc_primitive_type("Float", ctx.heap);
            let frame = ctx.task.call_stack.last_mut().unwrap();
            frame.registers[r_dst as usize] = Value::Ref(href);
            ExecutionResult::Continue
        }
        ...
        IntrinsicId::StringGetType => {
            let href = ctx
                .reflection
                .get_or_alloc_primitive_type("String", ctx.heap);
            let frame = ctx.task.call_stack.last_mut().unwrap();
            frame.registers[r_dst as usize] = Value::Ref(href);
            ExecutionResult::Continue
        }
                        ...
                        && impl_def.contract.table_id() == 10
                    {
                        if let Some(row) = impl_def.contract.row_index() {
                            ...
                            if contract_idx < module.contract_defs.len() {
                                ...
                                let name =
                                    writ_module::heap::read_string(&module.string_heap, cd.name)
                                        .unwrap_or("");
        ...
        IntrinsicId::TypeGetName => {
            ...
            let val = ctx.heap.get_field(type_href, 0).unwrap_or(Value::Void);
            let frame = ctx.task.call_stack.last_mut().unwrap();
            frame.registers[r_dst as usize] = val;
            ExecutionResult::Continue
        }
        ...
        IntrinsicId::MethodInfoInvoke => {
            ...
            if method_idx >= ctx.modules[method_module_idx].decoded_bodies.len() {
                return ExecutionResult::Crash(format!(
                    "MethodInfo.invoke: method index {} out of range",
                    method_idx
                ));
            }
            ...
            let reg_count = ctx.modules[method_module_idx].module.method_bodies[method_idx]
                .register_types
                .len();
        ...
        IntrinsicId::MethodInfoAttributes => {
            ...
            let (method_module_idx, method_idx) = match ctx
                .reflection
                .lookup_method_identity(mi_href)
            {
                Some(id) => id,
                None => {
                    return ExecutionResult::Crash("MethodInfoAttributes: not a MethodInfo".into());
                }
            };
            ...
            let mut attr_data: Vec<(String, Vec<writ_module::attr::AttrValue>)> = Vec::new();
            {
                ...
                for row in &module.attribute_defs {
                    ...
                    if row.owner.table_id() != TableId::MethodDef.as_u8() {
                        continue;
                    }
                    if row.owner.row_index() != Some(target_row) {
                        continue;
        ...
        IntrinsicId::FieldInfoAttributes => {
                ...
                .lookup_field_identity(fi_href)
            {
                Some(id) => id,
                None => {
                    return ExecutionResult::Crash("FieldInfoAttributes: not a FieldInfo".into());
                }
            };
            let (field_start, _) =
                ReflectionIndex::typedef_field_range_pub(ctx.modules, module_idx, typedef_idx);
            ...
            let mut attr_data: Vec<(String, Vec<writ_module::attr::AttrValue>)> = Vec::new();
            {
                ...
                for row in &module.attribute_defs {
                    ...
                    if row.owner.table_id() != TableId::FieldDef.as_u8() {
                        continue;
                    }
                    if row.owner.row_index() != Some(target_row) {
                        continue;
